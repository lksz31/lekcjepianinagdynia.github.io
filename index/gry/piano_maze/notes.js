...
const ALL_NOTE_DEFS=[
  ...
  {id:'c',freq:130.81,name:'c',clef:'B',staffY_G:null,staffY_B:5},
  {id:'d',freq:146.83,name:'d',clef:'B',staffY_G:null,staffY_B:4},
  {id:'e',freq:164.81,name:'e',clef:'B',staffY_G:null,staffY_B:3},
  {id:'f',freq:174.61,name:'f',clef:'B',staffY_G:null,staffY_B:2},
  {id:'g',freq:196.00,name:'g',clef:'B',staffY_G:null,staffY_B:1},
  {id:'a',freq:220.00,name:'a',clef:'B',staffY_G:null,staffY_B:0},
...
function resolveDisplayProps(n){
  let displayClef=n.clef,staffY=n.clef==='G'?n.staffY_G:n.staffY_B;
...
function buildActiveNotes(){
  const key=getRangeKey(),ids=NOTE_RANGE_MAP[key]||NOTE_RANGE_MAP['G_small'];
  ...
  MN=['N','E','S','W'].map(dir=>{
    const g=groups[dir];if(!g||!g.length)return null;
...
function updateRangeInfoBox(){
  const key=getRangeKey(),el=document.getElementById('range-info-text');
  if(el)el.textContent=NOTE_RANGE_LABELS[key]||'';
}
function noteSVG(n,W,H,showLabel){
  W=W||80;H=H||92;
  const dir=n.dir,c=noteColor(n,dir),isB=n.displayClef==='B';
  const scale=W/80,sg=Math.round(9*scale),staffTop=Math.round(18*scale);
  const noteX=W*0.65,staffY=n.staffY,noteY=staffTop+(staffY*sg/2);
  const trebleFS=Math.max(20,Math.round(W*0.60)),bassFS=Math.max(14,Math.round(W*0.33));
  ...
  if(noteY<topLine-sg*0.5){let ly=topLine-sg;while(ly>=noteY-sg*0.3){s+=`<line x1="${noteX-ledW}" y1="${ly}" x2="${noteX+ledW}" y2="${ly}" stroke="${c}" stroke-width="${Math.max(0.8,1.2*scale)}"/>`;ly-=sg;}}
  const nrx=Math.max(4,Math.round(6.5*scale)),nry=Math.max(3,Math.round(4.5*scale));
  s+=`<ellipse cx="${noteX}" cy="${noteY}" rx="${nrx}" ry="${nry}" fill="${c}" transform="rotate(-15,${noteX},${noteY})"/>`;
  const stemUp=noteY>staffTop+sg*2,sx=stemUp?noteX+nrx*0.9:noteX-nrx*0.9,stemLen=Math.max(18,Math.round(26*scale));
  const sy2=noteY+(stemUp?-stemLen:stemLen);
  s+=`<line x1="${sx}" y1="${noteY}" x2="${sx}" y2="${sy2}" stroke="${c}" stroke-width="${Math.max(1,1.5*scale)}"/>`;
  if(showLabel){const lblSz=Math.max(7,Math.round(9*scale));s+=`<text x="${W/2}" y="${H-4}" font-size="${lblSz}" fill="${c}" text-anchor="middle" font-family="Cinzel,serif" font-weight="700">${n.name} ${ARR[dir]}</text>`;}
  s+='</svg>';return s;
}
...
function shuffleTileOrder(){for(let i=tileOrder.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[tileOrder[i],tileOrder[j]]=[tileOrder[j],tileOrder[i]];}}
function reshuffleNotes(){shuffleTileOrder();buildActiveNotes();buildNoteTiles();refreshSideBar();}
function buildNoteTiles(){
  shuffleTileOrder();const bar=document.getElementById('note-bar');
  const show=controlMode==='tiles'||controlMode==='both';
  if(!show){bar.innerHTML='';bar.className='note-bar';return;}
  bar.innerHTML='';
  const bW=window.innerWidth,tileW=Math.min(82,Math.floor((bW-80)/4)),tileH=noteLabels?Math.min(102,tileW+14):Math.min(94,tileW+10);
  tileOrder.forEach(dir=>{
    const n=MN.find(x=>x.dir===dir);if(!n)return;
    const tt=document.createElement('div');tt.className='note-tile';tt.dataset.dir=dir;tt.innerHTML=noteSVG(n,tileW,tileH,noteLabels);
    tt.addEventListener('mousedown',()=>startPress(dir,tt));tt.addEventListener('mouseup',stopPress);tt.addEventListener('mouseleave',stopPress);
    tt.addEventListener('touchstart',e=>{e.preventDefault();startPress(dir,tt);},{passive:false});
    tt.addEventListener('touchend',stopPress);tt.addEventListener('touchcancel',stopPress);
    if(controlMode==='mic')tt.classList.add('mic-only');
    bar.appendChild(tt);
  });
  bar.className='note-bar show';
}
function startPress(dir,tile){
  if(won||!mazeData)return;if(controlMode==='mic')return;
  stopPress();activeTile=tile;tile.classList.add('pressing');move(dir);reshuffleNotes();
  pressTimer=setInterval(()=>{if(!won&&mazeData){move(dir);reshuffleNotes();}else stopPress();},200);
}
function stopPress(){clearInterval(pressTimer);pressTimer=null;if(activeTile){activeTile.classList.remove('pressing');activeTile=null;}}
function buildCtrlTable(container){
  ...
  MN.forEach(n=>{const col=coloredNotes?DIR_COLORS[n.dir]:NOTE_COLOR_DEFAULT;h+=`<tr><td style="color:${col}">${n.name}</td><td>${ARR[n.dir]}</td></tr>`;});
